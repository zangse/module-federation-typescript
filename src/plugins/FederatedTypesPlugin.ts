...
export class FederatedTypesPlugin {
  ...
  apply(compiler: Compiler) {
    ...
    const { disableDownloadingRemoteTypes, disableTypeCompilation, typescriptFolderName } =
    ...
    if (!disableDownloadingRemoteTypes) {
      ...
      compiler.hooks.watchRun.tapAsync(PLUGIN_NAME, async (compilation, callback) => {
        ...
        if (compilation?.modifiedFiles?.values) {
          const modifiedFiles = Array.from(compilation.modifiedFiles.values())
          const files = modifiedFiles.filter((filename) => !filename.includes(typescriptFolderName))
          if (files?.length > 0) {
            await importRemotes(callback);
          } else {
            callback()
  ...
  private async importRemoteTypes() {
    console.log('importRemoteTypes', this.options.federationConfig.remotes);
  ...
  async processRequest(compiler: Compiler, requestInfo: any) {
    this.logger.log('processRequest',compiler,requestInfo);
    await this.importRemoteTypes()
  }
}
